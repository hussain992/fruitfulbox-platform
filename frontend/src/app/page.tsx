...
export default function Home() {
  return (
    <main className="w-full overflow-x-hidden bg-[#fffdf4]">
      ...
      <section className="relative isolate overflow-hidden border-b border-amber-200 bg-[radial-gradient(circle_at_top_left,_rgba(255,208,75,0.38),_transparent_32%),linear-gradient(135deg,_#fff7cf_0%,_#fff3b5_30%,_#ffe7a0_58%,_#ffcf66_100%)] min-h-screen flex items-center">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:py-10 md:px-6 md:py-16 lg:gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div className="max-w-2xl">
            ...
            <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-6xl font-black leading-tight tracking-tight text-stone-900">
              Summer&apos;s best mangoes, delivered across Pune.
            </h1>

            <p className="mt-3 sm:mt-5 max-w-xl text-sm sm:text-base leading-7 text-stone-700 md:text-lg">
            ...
            <div className="mt-6 sm:mt-8 grid gap-3 max-h-80 overflow-hidden md:max-h-none md:grid-cols-3">
              {spotlightPoints.map(({ title, description, icon: Icon }) => (
                <div
                  key={title}
                  className="rounded-2xl border border-white/60 bg-white/65 p-3 sm:p-4 shadow-[0_12px_40px_rgba(120,53,15,0.08)] backdrop-blur"
                >
                  <Icon className="h-4 sm:h-5 w-auto text-amber-700" />
                  <h2 className="mt-2 sm:mt-3 text-xs sm:text-sm font-bold text-stone-900">
                    {title}
                  </h2>
                  <p className="mt-1 text-xs sm:text-sm leading-5 sm:leading-6 text-stone-700">
                    {description}
                  </p>
                </div>
              ))}
          ...
          <div className="relative mx-auto w-full max-w-xl">
            ...
            <div className="hidden md:visible relative overflow-hidden rounded-[2rem] border border-white/70 bg-[#fffaf0] p-4 shadow-[0_30px_80px_rgba(120,53,15,0.18)]">
      ...
      <section className="border-b border-amber-100 bg-white hidden md:block">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 py-10 md:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.22em] text-amber-700">
              Why order from us
            </p>
            <h2 className="mt-2 sm:mt-3 text-lg sm:text-2xl font-black text-stone-900 md:text-3xl lg:text-4xl">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {trustPoints.map((point) => (
              <div
                key={point}
                className="rounded-3xl border border-amber-100 bg-[#fff9ea] p-2 md:p-5 text-sm leading-6 text-stone-700 shadow-[0_12px_30px_rgba(120,53,15,0.06)]"
              >
                <CheckCircle2 className="mb-1 sm:mb-3 h-5 w-5 text-amber-700" />
                {point}
              </div>
            ))}
          </div>
      ...
      >
        <div className="mx-auto max-w-7xl">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-2 sm:mt-3 text-2xl sm:text-3xl font-black text-stone-900 md:text-4xl lg:text-5xl">
              Choose your mango mood.
            </h2>
            <p className="mt-2 sm:mt-4 text-sm leading-6 sm:text-base sm:leading-7 text-stone-600">
            ...
            {mangoProducts.map((product) => (
              ...
              >
                <div className="relative aspect-[6/5] sm:aspect-square overflow-hidden bg-[linear-gradient(160deg,_#fff9df_0%,_#ffe59d_60%,_#ffd261_100%)]">
                  ...
                  />
                  <div className="absolute left-2 sm:left-4 top-2 sm:top-4 rounded-full bg-white/90 px-2 py-0.5 text-xs font-semibold text-stone-800">
                    {product.isAvailable
                      ? "Available now"
                      : "Enquire for stock"}
                  </div>
                ...
                <div className="flex flex-1 flex-col p-3 sm:p-5">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-sm sm:text-lg md:text-xl font-bold text-stone-900 line-clamp-2">
                      {product.title}
                    </h3>
                    <span className="hidden sm:inline-flex rounded-full bg-amber-100 px-2 sm:px-3 py-1 text-xs font-semibold uppercase tracking-wide text-amber-800 flex-shrink-0">
                      Mango
                    </span>
                  </div>

                  <p className="mt-1 sm:mt-3 line-clamp-2 sm:line-clamp-4 text-xs sm:text-sm leading-5 sm:leading-6 text-stone-600">
                    {product.description}
                  </p>
                  ...
                  <div className="mt-2 sm:mt-5 flex items-end justify-between gap-2 sm:gap-4">
                    <div>
                      ...
                      <p className="mt-0.5 sm:mt-1 text-lg sm:text-2xl font-black text-stone-900">
                        {product.price.discounted}
                      </p>
                      <p className="hidden sm:block text-xs sm:text-sm text-stone-400 line-through">
                  ...
                  <div className="mt-3 sm:mt-6 flex gap-2 sm:gap-3">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full inline-flex items-center justify-center rounded-full bg-amber-500 px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-semibold text-stone-950 transition hover:bg-amber-400"
                    >
                      Order Now
                    </a>
                    <Link
                      href={`/${product.category}/${product.slug}`}
                      className="hidden sm:inline-flex flex-1 items-center justify-center rounded-full border border-stone-900 px-4 py-3 text-sm font-semibold text-stone-900 transition hover:bg-stone-900 hover:text-white"
                    >
                      Details
                    </Link>
                  </div>
      ...
      <section className="bg-stone-950 px-4 py-8 md:py-14 text-white md:px-6 hidden md:block">
        <div className="mx-auto grid max-w-7xl gap-6 md:gap-8 rounded-[2rem] border border-white/10 bg-[linear-gradient(145deg,_rgba(251,191,36,0.12),_rgba(24,24,27,0.96))] p-6 md:p-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.22em] text-amber-300">
              Delivery and ordering
            </p>
            <h2 className="mt-2 sm:mt-3 text-2xl sm:text-3xl font-black md:text-3xl lg:text-4xl">
              Keep the buying journey simple during mango season.
            </h2>
            <p className="mt-3 sm:mt-4 max-w-xl text-xs sm:text-sm leading-6 sm:leading-7 text-stone-300 md:text-sm lg:text-base">
      ...
      <section className="border-y border-amber-100 bg-[#fff8df] px-4 py-8 md:py-14 md:px-6">
        <div className="mx-auto max-w-5xl rounded-2xl sm:rounded-[2rem] border border-amber-200 bg-white px-4 py-6 sm:px-6 sm:py-10 text-center shadow-[0_24px_60px_rgba(120,53,15,0.08)] md:px-10">
          <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.22em] text-amber-700">
            Order today
          </p>
          <h2 className="mt-2 sm:mt-3 text-2xl sm:text-3xl font-black text-stone-900 md:text-4xl lg:text-5xl">
          ...
          <p className="mx-auto mt-2 sm:mt-4 max-w-2xl text-sm sm:text-base leading-6 sm:leading-7 text-stone-600">

...
export default function Header() {
        ...
        {isOpen && (
          ...
          >
            <div className="space-y-4 px-4 py-4">
              <div className="rounded-2xl border border-amber-200 bg-white/80 p-4 text-sm text-stone-700">
                <div className="flex items-center gap-2 font-semibold text-amber-800">
                  <Sparkles className="h-4 w-4" />
                  Mango season picks are live
                </div>
                <p className="mt-2 leading-6">
                  Browse mangoes, then message on WhatsApp for stock and delivery confirmation.
                </p>
              </div>
              ...
              <div className="space-y-2">
                {links.map((link) => (
                  <Link
                    ...
                    className="block rounded-xl px-3 py-2 text-sm font-medium text-stone-700 transition hover:bg-white hover:text-amber-800"
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              <a
                ...
                onClick={() => setIsOpen(false)}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-stone-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-stone-800"
              >
  ...
  return (
    <header className="sticky top-[-36px] z-50 border-b border-amber-200 bg-[linear-gradient(180deg,_rgba(255,248,223,0.98)_0%,_rgba(255,253,244,0.97)_100%)] shadow-[0_10px_35px_rgba(120,53,15,0.08)] backdrop-blur">
      <div className="border-b border-amber-200 bg-[linear-gradient(90deg,_#fff2b4_0%,_#ffd66b_52%,_#ffefbc_100%)] px-4 py-2 text-xs text-stone-800 md:text-sm">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-5 gap-y-1 font-medium">
          <span className="inline-flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-amber-800" />
            Mango season specials now on the homepage
          </span>
          <span className="inline-flex items-center gap-2">

...
export default function ProductCard({ image, title, description, slug }: ProductCardProps) {
  ...
  return (
    ...
    <div className="bg-orange-100 p-3 mx-4 sm:mx-0 md:p-6 rounded-xl shadow-md text-center transition-transform duration-300 hover:scale-105 hover:shadow-xl hover:bg-yellow-200">
  ...
  />
      ...
      <h3 className="text-lg font-bold mb-2 line-clamp-2 h-15 sm:h-13">{title}</h3>
      {description && <p className="text-md text-gray-700 line-clamp-2">{description}</p>}
    </div>
    </Link>
  );
}